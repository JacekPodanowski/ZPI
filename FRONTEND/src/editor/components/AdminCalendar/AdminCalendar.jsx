...
const HOURS = Array.from({ length: 16 }, (_, idx) => 6 + idx); // 06:00 - 21:00
        ...
        >
            <Box className={styles.topBar}>
                <Stack direction="row" alignItems="center" spacing={1.5}>
                    ...
                    <Chip label="tryb zaawansowany" size="small" sx={{ background: brand.subtle, color: brand.text }} />
                </Stack>
                ...
                {daysForGrid.map((entry, index) => {
                    ...
                    return (
                        <Paper
                            key={dayKey}
                            elevation={0}
                            className={[styles.dayTile, entry.events.length === 0 && entry.availability.length === 0 ? styles.dayTileUnavailable : '', entry.external.length > 0 ? styles.dayTileExternal : ''].join(' ')}
                            style={{
